    ...
    public:
        ...
        void InitializeRemainingVotes() {
            ...
            for (auto it = candidates.begin(); it != candidates.end(); it++) {
                //std::cout<<(*it)->GetName()<<" is in party "<<name<<" votes: "<<(*it)->GetNumVotes()<<"\n";
                int canvotes = (*it)->GetNumVotes();
                totalVotes += canvotes;
                // int canvotes = (*it)->GetNumVotes();
                // remainingVotes += canvotes;
                //std::cout<<(*it)->GetName()<<" is in party "<<name<<" votes: "<<canvotes<<" totalpartyvotes "<<totalVotes<<"\n";
            }

            //remainingVotes = totalVotes;
            // yo why tf is this not setting remainingVotes?
            //std::cout<<"Nowtotalvotesis "<<totalVotes<<"\n";
        ...
        void UpdateRank(Audit audit) {
            ...
            for (auto it = candidates.begin(); it != candidates.end(); it++) {
                ...
                for(int j = 0; j < candidates.size(); j++) {
                    // Empty slot (I had previous problems with candidates[-1] lol)
                    if (ranks[j] == -1) {
                        ranks[j] = i;
                        break;
                    }
            ...
            for(int i = 0; i < candidates.size(); i++) {
                ...
                audit.auditPrint(candidates[i]->GetName()+" has rank "+std::to_string(candidates[i]->GetCurPartyRank())+".");
            }
